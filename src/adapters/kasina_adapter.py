...
class KasinaAdapter:
    ...
    def _enqueue_collect(self, item: dict, model_no: str) -> bool:
        ...
        return enqueue_collect(
            ...
            brand_hint=item.get("brandNameKo")
            or item.get("brandNameEn")
            or item.get("_brand_name")
            or "",
            name_hint=item.get("productName") or item.get("productNameEn") or "",
            source=self.source_name,
            source_url=_build_url(pno) if pno is not None else "",
        )
    ...
    async def match_to_kream(
        ...
        for item in products:
            ...
            if kream_row is None:
                ...
                if self._enqueue_collect(item, mgmt):
