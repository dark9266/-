...
크롤러 결과만 보고 PASS 판정 금지. 반드시 **원본 데이터와 교차검증**:

1. 크롤러로 `get_product_detail()` 호출 → 사이즈 리스트 수집
2. 같은 상품의 raw HTML/API를 **크롤러 우회해서** 직접 httpx로 fetch
3. raw 응답에서 독립적으로 사이즈/재고 파싱 (정규식 or JSON 직접 추출)
4. 두 결과를 비교 — 불일치 0건이어야 PASS

이유: mock 테스트 935개 통과해도 EQL/반스/살로몬 3곳이 사이즈 0개 반환하는 버그를 못 잡았음 (2026-04-16 사고).

## 품목 다양화 (필수)

테스트 상품 선정 시 **신발만 금지**:
- 신발 1건 + 의류 1건 (또는 액세서리) 최소 조합
- 남성/여성/키즈 혼합 권장
- 사이즈 포맷이 다른 품목 포함 (mm숫자 vs XS/S/M/L vs FREE)
...
| **사이즈 교차검증** | **크롤러 vs raw 불일치 0건** | **1건이라도 불일치** |
| 가격 | 양수 (> 0) | 0 또는 음수 |
| 응답시간 | < 10초 | >= 10초 |
| 모델매칭 | 정확 일치 | 불일치 |
