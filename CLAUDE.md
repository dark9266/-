...
```
크림 DB (47k) → 우선순위 큐 → 소싱처 4곳 병렬 검색 → 모델번호 매칭 → 수익 분석 → Discord 알림
```
...
| 티어 | 주기 | 역할 | 모듈 |
|------|------|------|------|
| **Tier 0** | 5분/50건 | 연속 배치 스캔 — 47k 전체 순환 (구현 예정) | `src/continuous_scanner.py` |
...
### hot/warm/cold 우선순위 큐 (구현 예정)

| 등급 | 조건 | 재스캔 주기 | 검색 소싱처 | 규모 |
|------|------|-----------|-----------|------|
| **hot** | 거래량 ≥ 10 | 2시간 | 4곳 전부 | ~2,000건 |
| **warm** | 거래량 3~9 | 8시간 | 무신사+나이키+29CM | ~8,000건 |
| **cold** | 거래량 < 3 | 48시간 | 무신사+나이키 | ~37,000건 |

DB 컬럼: `next_scan_at`, `scan_priority` (kream_products 테이블 확장 예정)

## Key Modules

### 스캐너
- `src/reverse_scanner.py` — 역방향 스캐너. 크림 hot → 소싱처 4곳 병렬 검색 → 사이즈 교차 매칭(sell_now>0 필수) → 수익 분석
...
- `src/continuous_scanner.py` — **(구현 예정)** next_scan_at 기반 47k 연속 배치 스캔
...
- `src/crawlers/nike.py` — 나이키 공식몰. `__NEXT_DATA__` JSON 파싱 (selectedProduct 구조)
...
| `queue-inspector` | **(v2 예정)** 스캔 큐 상태 진단, 적체/소화율 | Read + Bash(SELECT) |
| `scan-debugger` | **(v2 예정)** 상품별 스캔 파이프라인 추적 | Read + Bash(GET+SELECT) |
...
| `/queue` | **(v2 예정)** 스캔 큐 현황 — priority별 분포, 적체, ETA |
| `/trace` | **(v2 예정)** 특정 모델번호 스캔 파이프라인 추적 |
| `/health` | **(v2 예정)** 소싱처 4곳 헬스체크 — 응답시간, 서킷브레이커 |
...
- 소싱처별: 무신사(안정), 29CM(안정), 나이키(안정), 아디다스(WAF 주의)
