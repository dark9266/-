#!/bin/bash
# Stage 2 완주 자동 판정 — 2026-04-22 13:35 KST 실행 예정.
# 헬스체크 4종 + decision_log 2h 집계 + Discord 웹훅 발사.
set -u
...
# 헬스체크 2/4 + Stage 2 집계: decision_log
SQL=$(cat <<EOF
import sqlite3
...
# kream_api_calls 2h (health #3)
cur.execute("SELECT COUNT(*) FROM kream_api_calls WHERE ts >= datetime(?,'unixepoch')", (t0,))
print(f"KREAM_CALLS_STAGE2={cur.fetchone()[0]}")

# decision_log Stage 2 집계
...
print(f"DECISION_TOTAL_STAGE2={cur.fetchone()[0]}")
EOF
)
eval $(python3 -c "$SQL")
...
elif [ "$DECISION_TOTAL_STAGE2" -lt "50" ]; then
  VERDICT="🟡 WARN — 파이프라인 활동 저조 (Stage2 total=$DECISION_TOTAL_STAGE2)"
else
  VERDICT="🟢 PASS — Stage 3 승격 가능"
fi

# Discord 메시지 조립
MSG=$(cat <<EOF
**Stage 2 (2h) 자동 판정 — 2026-04-22 13:35 KST**

판정: $VERDICT
봇: $BOT_STATUS
가동 시각: 11:35:30 ~ 13:35:00 (약 2h)

**헬스체크**
- 알림 24h: $ALERTS_24H 건
- 크림 호출 Stage 2: $KREAM_CALLS_STAGE2 회
- Discord 실발사 Stage 2: **$DISCORD_SENT_STAGE2 건** (매수+강력매수만)
...
- total: $DECISION_TOTAL_STAGE2

다음 세션에서 "이어서" → Stage 3 (6h) 진입 판단.
EOF
)
...
curl -s -X POST -H "Content-Type: application/json" -d "$PAYLOAD" "$WEBHOOK" > logs/stage2_webhook_response.txt
echo "[$(date '+%Y-%m-%d %H:%M:%S')] Stage 2 check 완료 — verdict: $VERDICT" >> logs/stage2_check.out
