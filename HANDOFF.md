...
```
크림 인기상품 수집 → 모델번호로 무신사 검색 → 사이즈별 가격 비교
...
| 파일 | 역할 |
|------|------|
| `scanner.py` | 전체 스캔 파이프라인 조율. `scan_keyword()`, `auto_scan()` 메서드가 핵심 |
...
- [x] **크림 인기상품 수집** — 인기순/판매순/급상승 3가지 소스
...
### 복합 모델번호 매칭 실패
크림의 모델번호가 `315122-111/CW2288-111`처럼 슬래시로 구분된 경우, 무신사 검색에서 0건이 나옴. `_match_model_with_slash()`가 있지만 무신사가 이런 포맷을 인식하지 못함. **각 컴포넌트를 분리하여 개별 검색하는 로직 필요.**
...
### 우선순위 높음
1. **복합 모델번호 분리 검색** — `315122-111/CW2288-111` → `315122-111`과 `CW2288-111` 각각 검색
2. **모델번호 매핑 테이블** — 크림↔무신사 간 모델번호가 다른 경우를 위한 수동/자동 매핑 DB
3. **무신사 외 리테일 크롤러 추가**
   - 카시나 (kasina.co.kr)
   - 나이키 코리아 (nike.com/kr)
   - 29CM
   - ABC마트
4. **자동 로그인** — 크림/무신사 세션 만료 시 자동 재로그인

### 우선순위 중간
5. **GitHub 연동** — 코드 원격 저장소 + CI/CD
6. **카테고리 확장** — 스니커즈 외 의류/액세서리/테크
7. **매칭 정확도 개선** — 모델번호 + 상품명 유사도 복합 매칭
8. **알림 필터 커스터마이징** — 사용자별 브랜드/사이즈/ROI 필터
9. **대시보드 웹 UI** — 실시간 모니터링 웹 인터페이스

### 우선순위 낮음
10. **멀티 계정** — 여러 크림/무신사 계정 순환 사용 (레이트리밋 분산)
11. **가격 예측** — 가격 추세 기반 매수 타이밍 추천
12. **Docker 컨테이너화** — 배포 자동화
13. **텔레그램 봇** — 디스코드 외 알림 채널
...
AUTO_SCAN_MAX_PRODUCTS=50             # 1회 스캔 최대 상품 수
...
무신사 매칭:        0건 ⚠️ (인기상품이 무신사 미판매 모델)
수익 기회:          0건
```

**매칭 실패 원인:** 크림 인기 스니커즈(에어포스1 등)는 무신사에서 판매하지 않거나, 복합 모델번호(`315122-111/CW2288-111`)를 무신사 검색엔진이 인식하지 못함. 파이프라인 자체는 정상 작동하며, 무신사에서 판매 중인 모델을 대상으로 하면 매칭+수익분석이 정상 동작함.
